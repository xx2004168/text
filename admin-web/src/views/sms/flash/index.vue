  ...
  export default {
    ...
    filters: {
      formatActiveStatus(row) {
        ...
        // console.log("活动开始时间：" + Date.parse(row.startDate));
        // console.log("活动结束时间：" + Date.parse(row.endDate));
        if (nowTime >= Date.parse(row.startDate) && nowTime <= Date.parse(row.endDate)) {
          return '活动进行中';
        } else if (nowTime > Date.parse(row.endDate)) {
          return '活动已结束';
        } else {
          return '活动未开始';
        }
